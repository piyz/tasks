...
public class Queen extends Figure {
    ...
    boolean isValidMove(Cell from, Cell to, Board board) {
        if (to.getY() > -1 && to.getY() < 8 && to.getX() > -1 && to.getX() < 8){
            ...
            checkSingleCell(board, new Cell(from.getX() - 1, from.getY()), moves);
            checkSingleCell(board, new Cell(from.getX() + 1, from.getY()), moves);
            checkSingleCell(board, new Cell(from.getX(), from.getY() - 1), moves);
            checkSingleCell(board, new Cell(from.getX(), from.getY() + 1), moves);

            checkSingleCell(board, new Cell(from.getX() - 1, from.getY() + 1), moves);
            checkSingleCell(board, new Cell(from.getX() - 1, from.getY() - 1), moves);
            checkSingleCell(board, new Cell(from.getX() + 1, from.getY() + 1), moves);
            checkSingleCell(board, new Cell(from.getX() + 1, from.getY() - 1), moves);
